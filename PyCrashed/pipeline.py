...
class Dataset:
    ...
    def load(method="train"):
        ...
        if method in ["validate", "val"]:
            ...
            ds = ds.take(int(n_items * Dataset._props["N_VAL"]))
        elif method == "test":
            ds = ds.skip(int(n_items * (Dataset._props["N_TRAIN"] + Dataset._props["N_VAL"])))
            ds = ds.take(int(n_items * Dataset._props["N_TEST"]))
        else:
